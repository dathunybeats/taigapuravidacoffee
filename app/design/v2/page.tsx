...
export const metadata: Metadata = {
  title: "Design Review — Version 2 (Cream/Maroon)",
  robots: { index: false },
};

// V2 palette — cream + maroon, inspired by the menu PDFs
// Dark bg:    #3D0A0A  (deep maroon replaces espresso brown)
// Light bg:   #FAE8D0  (warm peach cream replaces cool linen)
// Heading:    #7B1F1F  (maroon on light), #FAE8D0 (cream on dark)
// Eyebrow:    #C4622D  (terracotta orange — kept as accent, it pops against maroon)
// Body light: #4A1C1C  (dark maroon-brown)
// Body dark:  rgba(250,232,208,0.5)
// Cards:      #FFF5EC  (warm white)
// Borders:    rgba(210,180,155,0.6)
// CTA dark:   #7B1F1F  (maroon button, cream text)
// CTA light:  #7B1F1F  (maroon button, cream text)
...
export default function V2() {
  return (
    <div className="min-h-screen" style={{ background: "#FAE8D0" }}>

      {/* ── Comparison bar ── */}
      <div className="sticky top-0 z-50 flex items-center justify-between gap-4 px-5 py-3 border-b" style={{ background: "#3D0A0A", borderColor: "rgba(250,232,208,0.12)" }}>
        <div className="flex items-center gap-3">
          <span className="rounded-full px-3 py-1 text-[10px] tracking-[0.2em] uppercase" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>
            Version 2
          </span>
          <span className="text-xs tracking-wider hidden sm:block" style={{ color: "rgba(250,232,208,0.45)" }}>
            Cream + maroon · menu PDF palette
          </span>
        </div>
        <a href="/design/v1" className="text-xs tracking-[0.2em] uppercase transition-colors hover:opacity-100" style={{ ...btn, color: "rgba(250,232,208,0.5)" }}>
          See Version 1 →
        </a>
      </div>

      {/* ── Nav ── */}
      <nav style={{ background: "#3D0A0A" }} className="px-5 sm:px-12 py-4 flex items-center justify-between">
        ...
        <div className="hidden sm:flex gap-8">
          {["Services", "About", "Contact"].map(l => (
            <span key={l} className="text-sm" style={{ ...btn, color: "rgba(250,232,208,0.6)" }}>{l}</span>
          ))}
        </div>
        <span className="rounded-full px-5 py-2.5 text-sm" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>Get a Quote</span>
      </nav>

      {/* ── Hero ── */}
      <section className="relative flex flex-col justify-center overflow-hidden" style={{ background: "#3D0A0A", minHeight: "72vh" }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="absolute right-0 top-0 h-full w-[50%] hidden sm:block pointer-events-none" style={{ background: "linear-gradient(to right, #3D0A0A 0%, #3D0A0A 20%, transparent 70%)" }} />
        <div className="relative z-10 max-w-6xl mx-auto w-full px-5 sm:px-12 py-20 sm:py-28">
          ...
          <h1 className="leading-none text-5xl sm:text-6xl lg:text-7xl mb-5" style={{ color: "#FAE8D0" }}>
            Farm-to-Cup<br />Coffee<br />Catering.
          </h1>
          <p className="text-base sm:text-lg leading-7 max-w-lg mb-8 font-light" style={{ color: "rgba(250,232,208,0.5)" }}>
          ...
          <div className="flex flex-wrap gap-4">
            <span className="rounded-full px-9 py-4" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>Get a Quote</span>
            <span className="rounded-full px-9 py-4" style={{ ...btn, border: "1px solid rgba(250,232,208,0.25)", color: "#FAE8D0" }}>Our Story</span>
          </div>
        </div>
      </section>

      {/* ── Services ── */}
      <section className="py-20 sm:py-32" style={{ background: "#FAE8D0" }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-12">
          <div className="mb-14 sm:mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl leading-tight" style={{ color: "#7B1F1F" }}>Every event deserves<br className="hidden sm:block" /> exceptional coffee.</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
            {services.map(s => (
              <div key={s.title} className="rounded-2xl p-7 sm:p-10 flex flex-col" style={{ background: "#FFF5EC", border: "1px solid rgba(210,180,155,0.6)" }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <h3 className="text-2xl sm:text-3xl mb-3" style={{ color: "#7B1F1F" }}>{s.title}</h3>
                <p className="text-sm leading-6 font-light" style={{ color: "rgba(74,28,28,0.7)" }}>{s.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 sm:py-32" style={{ background: "#FAE8D0" }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 sm:gap-16 items-center">
            ...
            <div>
              <p className="text-xs tracking-[0.35em] uppercase mb-6" style={{ color: "#7B1F1F" }}>What We Bring</p>
              <h2 className="leading-tight text-4xl sm:text-5xl mb-6" style={{ color: "#7B1F1F" }}>
                A full specialty<br />bar experience,<br />custom to your event.
              </h2>
              <p className="text-base leading-7 font-light mb-8" style={{ color: "rgba(74,28,28,0.75)" }}>
              ...
              <ul className="space-y-3.5 mb-10">
                {whatWeBring.map(cat => (
                  <li key={cat} className="flex items-center gap-3">
                    <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: "#7B1F1F" }} aria-hidden="true" />
                    <span className="text-sm font-light" style={{ color: "#4A1C1C" }}>{cat}</span>
                  </li>
                ))}
              </ul>
              <span className="inline-flex items-center rounded-full px-7 py-3.5" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>
      ...
      {/* ── Corporate Events ── */}
      <section className="py-20 sm:py-32" style={{ background: "#3D0A0A" }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-12">
          <div className="mb-14 sm:mb-16">
            <p className="text-xs tracking-[0.35em] uppercase mb-5" style={{ color: "#C4622D" }}>Corporate Events</p>
            <h2 className="text-4xl sm:text-5xl leading-tight" style={{ color: "#FAE8D0" }}>Bring the café<br className="hidden sm:block" /> to your office.</h2>
          </div>
          ...
          <div className="mt-12 sm:mt-16 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
            <p className="text-base leading-7 font-light max-w-lg" style={{ color: "rgba(250,232,208,0.6)" }}>
            ...
            <span className="shrink-0 inline-flex items-center rounded-full px-7 py-3.5" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>Get a Quote</span>
          </div>
        </div>
      </section>

      {/* ── About teaser ── */}
      <section className="py-20 sm:py-32" style={{ background: "#FFF5EC" }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 sm:gap-16 items-center">
            <div>
              <p className="text-xs tracking-[0.35em] uppercase mb-6" style={{ color: "#C4622D" }}>Our Roots</p>
              <h2 className="text-4xl sm:text-5xl leading-tight mb-6" style={{ color: "#7B1F1F" }}>Sourced from<br />Costa Rica&apos;s<br />highlands.</h2>
              <p className="text-base leading-7 font-light mb-5" style={{ color: "rgba(74,28,28,0.7)" }}>
              ...
              <p className="text-base leading-7 font-light mb-10" style={{ color: "rgba(74,28,28,0.7)" }}>
              ...
              <span className="inline-flex items-center rounded-full px-7 py-3.5" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>Work With Us</span>
            </div>
            <div className="relative">
              ...
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/img-kids-cups.jpg" alt="Community enjoying Taïga coffee" className="absolute -bottom-6 -left-4 sm:-left-8 w-2/5 rounded-xl object-cover shadow-lg" style={{ aspectRatio: "1", border: "4px solid #FFF5EC" }} />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 sm:gap-16 items-start">
            <div>
              ...
              <h2 className="text-4xl sm:text-5xl leading-tight mb-5" style={{ color: "#FAE8D0" }}>Let&apos;s plan<br />your event.</h2>
              <p className="text-base leading-7 font-light mb-10" style={{ color: "rgba(250,232,208,0.45)" }}>
              ...
              <ul className="space-y-3.5">
                {["Weddings & private celebrations", "Corporate events & office setups", "Markets, pop-ups & festivals"].map(item => (
                  <li key={item} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-light" style={{ color: "rgba(250,232,208,0.55)" }}>{item}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="rounded-2xl p-6 sm:p-8" style={{ background: "#FAE8D0" }}>
              <p className="text-sm font-light mb-4" style={{ color: "rgba(74,28,28,0.4)" }}>Quote form lives here</p>
              <div className="space-y-3">
                {["Your name", "Email address", "Event date", "Guest count"].map(f => (
                  <div key={f} className="rounded-lg px-4 py-3 text-sm font-light" style={{ background: "#FFF5EC", border: "1px solid rgba(210,180,155,0.6)", color: "rgba(74,28,28,0.3)" }}>{f}</div>
                ))}
                <div className="rounded-full px-7 py-3.5 text-center text-sm mt-2" style={{ ...btn, background: "#7B1F1F", color: "#FAE8D0" }}>Send Request</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── Footer ── */}
      <footer className="py-12 sm:py-16" style={{ background: "#3D0A0A", borderTop: "1px solid rgba(250,232,208,0.1)" }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          <p className="text-xs" style={{ color: "rgba(250,232,208,0.2)" }}>© 2026 Taïga Pura Vida Coffee · Charlottesville, VA</p>
        </div>
      </footer>
